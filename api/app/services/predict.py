...
class PredictionService:
    ...
    def predict_2027(db: Session, code_insee: str, model_name: str):
        ...
        query = text(
            "SELECT city, years, statistics FROM communes_stats WHERE code_insee = :code"
        )
        ...
        def project(column_name):
            if column_name in ["Année", "Résultat"]:
                return None
        ...
        for l in labels_active:
            projections[l] = (
                (projections[l] / pop_active * 100) if pop_active > 0 else 0.0
            )
        for l in labels_menages:
            projections[l] = (
                (projections[l] / pop_enfants * 100) if pop_enfants > 0 else 0.0
            )
        ...
        if not os.path.exists(model_path):
            raise HTTPException(
                status_code=500, detail=f"Modèle binaire introuvable : {model_path}"
            )
        ...
        scores = {
            str(c): round(float(p) * 100, 2) for c, p in zip(model.classes_, proba)
        }
        ...
        return {
            ...
            "scores": scores,
            "top_features": dict(
                list(meta["feature_importances"].items())[:5]
            ),  # Top 5
