...
@router.get(
    "/", status_code=status.HTTP_200_OK, response_model=PaginatedCommuneResponse
)
async def get_and_search_communes(
    ...
    """
    total, data = CommuneService.get_all(
        db, skip=skip, limit=limit, search=search, light=light
    )
    return {
        "total": total,
        "page": (skip // limit) + 1,
...
@router.get(
    "/commune", status_code=status.HTTP_200_OK, response_model=List[CommuneResponse]
)
...
async def get_communes_by_department(
    ...
    Returns:
        List[CommuneResponse]: Liste des communes correspondantes
    """
    stats = CommuneService.get_by_department(
        db, department_code=department_code, year=year
    )
    return stats


@router.get("/communes/region/{region_code}")
def get_communes_by_region(
    region_code: str, year: str = "2022", db: Session = Depends(get_db)
):
    ...
    query = select(Communes).where(or_(*filters), Communes.years == year)
