...
def convert_columns_to_percentages(df, list_columns, divider_column):
    ...
    df_conv.loc[mask, list_columns] = (
        df_conv.loc[mask, list_columns].div(df_conv[divider_column], axis=0) * 100
    )
    df_conv[list_columns] = (
        df_conv[list_columns].fillna(0).replace([np.inf, -np.inf], 0)
    )
    return df_conv
...
def run_ingestion():
    ...
    df_train = convert_columns_to_percentages(
        df_train, cols_active, "Population_active"
    )
    ...
    df_train = convert_columns_to_percentages(
        df_train, cols_household, "Population avec enfants"
    )
    ...
    df_final = pd.DataFrame(
        {
            ...
            "pct_gauche": pd.to_numeric(
                df_communes["% gauche/Exp"], errors="coerce"
            ).fillna(0),
            "pct_centre": pd.to_numeric(
                df_communes["% centre/Exp"], errors="coerce"
            ).fillna(0),
            "pct_droite": pd.to_numeric(
                df_communes["% droite/Exp"], errors="coerce"
            ).fillna(0),
