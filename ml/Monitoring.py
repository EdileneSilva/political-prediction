...
def run_experiment(
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        train_metrics = compute_metrics(
            y_train, y_pred_train, label_names, prefix="train"
        )
        ...
        cm_df = pd.DataFrame(
            cm,
            index=[f"real_{g}" for g in labels],
...
def main():
    ...
    _, m = run_experiment(
            ...
            [
                ...
                (
                    ...
                    SMOTEENN(
                        random_state=RANDOM_STATE,
                        smote=SMOTE(
                            sampling_strategy={0: 10000, 2: 8000},
                            random_state=RANDOM_STATE,
                        ),
                    ),
                ),
        ...
        params={
            "model": "XGBoost",
            "strategy": "SMOTEENN",
            "smoteenn_centre": 10000,
            "smoteenn_gauche": 8000,
            **XGB_PARAMS,
        },
        labels=list(range(len(xgb_labels))),
        label_names=xgb_labels,
    ...
    summary = pd.DataFrame(results).sort_values("test_f1_macro", ascending=False)[
        [
            "run",
            "test_accuracy",
            "test_f1_macro",
